...
class TrainConfig(BaseConfig):
    ...
    def make_model_conf(self):
        if self.model_name == ModelName.beatgans_ddpm:
            ...
            # 封装model_conf
            self.model_conf = cls(
                attention_resolutions=self.net_attn,  # 16
                channel_mult=self.net_ch_mult,  # [1,2,4,8]
                conv_resample=True,
                dims=2,
                dropout=self.dropout,  # 0.1
                embed_channels=self.net_beatgans_embed_channels,  # 512
                enc_out_channels=self.style_ch,  # 512
                enc_pool=self.net_enc_pool,  # adaptivenonzero
                enc_num_res_block=self.net_enc_num_res_blocks,  # 2
                ...
                image_size=self.img_size,  # 64
                in_channels=3,
                model_channels=self.net_ch,  # 64
                num_classes=None,
                num_head_channels=-1,
                num_heads_upsample=-1,
                num_heads=self.net_beatgans_attn_head,  # 1
                num_res_blocks=self.net_num_res_blocks,  # 2
                num_input_res_blocks=self.net_num_input_res_blocks,
                out_channels=self.model_out_channels,  # 3
                resblock_updown=self.net_resblock_updown,  # True
                use_checkpoint=self.net_beatgans_gradient_checkpoint,  # False
                use_new_attention_order=False,
                resnet_two_cond=self.net_beatgans_resnet_two_cond,  # True
                resnet_use_zero_module=self.
                net_beatgans_resnet_use_zero_module,  # True
                latent_net_conf=latent_net_conf,
                resnet_cond_channels=self.net_beatgans_resnet_cond_channels,  # None
            )
        else:
