...
class BeatGANsUNetModel(nn.Module):
    def __init__(self, conf: BeatGANsUNetConfig):
        ...
        self.time_emb_channels = conf.time_embed_channels or conf.model_channels  # 128
        # time embedding
        ...
        # input_block_chans = [ch]   [[128,],[],[],[],[]]
        input_block_chans = [[] for _ in range(len(conf.channel_mult))]
        input_block_chans[0].append(ch)

        # number of blocks at each resolution 相当于构建了一个栈，用于记录每个分辨率的Resblock块数
            ...
            for _ in range(conf.num_input_res_blocks or conf.num_res_blocks):
                layers = [
                    ResBlockConfig(
                        ...
                        conf.dropout,  # 0.1
                        out_channels=int(mult * conf.model_channels),  # 128
                        dims=conf.dims,  # 2
                        use_checkpoint=conf.use_checkpoint,  # False
                        **kwargs,
                    ).make_model()
                ]
        ...
        # [1,1,2,3,4]  ——> {[128,128,128],[128,128,128],[128,256,256],[256,384,384],[384,512,512]}
...
class BeatGANsEncoderModel(nn.Module):
    ...
    def forward(self, x, t=None, return_2d_feature=False):
        ...
        if self.conf.use_time_condition:
            # timestep_embedding():an [N x dim] Tensor of positional embeddings.
            # 然后丢入神经网络中，实现了时间特征的嵌入.
