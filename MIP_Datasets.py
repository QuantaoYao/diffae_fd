...
if __name__ == '__main__':
    sssset = MIP_Datasets(root='./datasets/HUN_yqt_MIP_II', resize=256, mode='test')
    ...
    for idx, batch in enumerate(looooader):
        ...
        for i in range(4):
            if not os.path.exists(os.path.join(SAVE_PATH, '')):
                os.mkdir(os.path.join(SAVE_PATH, 'HUN_yqt_MIP_II'))
            save_demorph(real1[i], morph[i], real2[i],
                         path=os.path.join(SAVE_PATH, 'HUN_yqt_MIP_II'),
