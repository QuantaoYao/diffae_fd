...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        # print('Model params: %.2f M' % (model_size))
    ...
    def setup(self, stage=None) -> None:
        ...
        if stage == 'fit' or stage is None:
            self.train_data = FD_Datasets(root='../datasets/FD_Datasets', resize=128, mode='train')
            self.dev_data = FD_Datasets(root='../datasets/FD_Datasets', resize=128, mode='dev')
        if stage == 'test' or stage is None:
            self.test_data = FD_Datasets(root='../datasets/FD_Datasets', resize=128, mode='test')
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                real1, real2, morph, idxs = batch['real1'], batch['real2'], batch['morph'], batch['idxs']
                # print(f'real1.shape:{real1.shape}')
                # img = np.concatenate(
                #     (((real1[0].permute(1, 2, 0))*0.5 + 0.5).cpu(), ((real2[0].permute(1, 2, 0))*0.5 + 0.5).cpu(), ((morph[0].permute(1, 2, 0))*0.5 + 0.5).cpu()),
                #     axis=1)
                # plt.imshow(img)
                # plt.show()
                # print(1)
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                t_real1, weight_real1 = self.T_sampler.sample(len(x_start_real1), x_start_real1.device)
                losses_real1 = self.sampler.training_losses(model=self.model,
                                                            x_start=x_start_real1,
                                                            t=t_real1)
                t_real2, weight_real2 = self.T_sampler.sample(len(x_start_real2), x_start_real2.device)
                losses_real2 = self.sampler.training_losses(model=self.model,
                                                            x_start=x_start_real2,
                                                            t=t_real2)
                t_morph, weight_morph = self.T_sampler.sample(len(x_start_morph), x_start_morph.device)
                losses_morph = self.sampler.training_losses(model=self.model,
                                                            x_start=x_start_morph,
                                                            t=t_morph)
            ...
            losses = (losses_real1['loss'].mean() + losses_real2['loss'].mean() + losses_morph['loss'].mean()) / 3
    ...
    def validation_step(self, batch, batch_idx):
        ...
        losses_morph = self.sampler.training_losses(model=self.model,
                                                    x_start=x_start_morph,
                                                    t=t_morph)
        val_loss = (losses_real1['loss'].mean() + losses_real2['loss'].mean() + losses_morph['loss'].mean()) / 3
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    checkpoint = ModelCheckpoint(
        monitor='val_loss',
        filename='sample-faceDemorphing-{epoch:02d}-{val_loss:02f}',
        dirpath=f'{conf.logdir}',
        save_last=True,
        save_top_k=2, )
    # 根据val_loss设置早停
    #    early_stopping = pytorch_lightning.callbacks.EarlyStopping('val_loss')
